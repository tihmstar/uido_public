...
void hmac(__global word *K, const word K_len_bytes, __global const word *m, const word m_len_bytes, __global word *output)
{
...
void pbkdf2_iOS(__global char *inbuffer, ulong length, __global const char *salt, __global char *outbuffer){
    hmac((__global word *)inbuffer, length, (__global const word *)&salt[0], 24, (__global word *)outbuffer);
    hmac((__global word *)inbuffer, length, (__global const word *)&salt[24], 24, (__global word *)&outbuffer[20]);
}
...
//    uint pSize = get_global_size(0); // eg. 256
    uint pSize = get_global_size(0) * get_global_size(1);
    ...
    T1 = U[7*pSize] ^ U[4*pSize];
    T2 = U[7*pSize] ^ U[2*pSize];
    T3 = U[7*pSize] ^ U[1*pSize];
    T4 = U[4*pSize] ^ U[2*pSize];
    T5 = U[3*pSize] ^ U[1*pSize];
    T6 = T1 ^ T5;
    T7 = U[6*pSize] ^ U[5*pSize];
    T8 = U[0*pSize] ^ T6;
    T9 = U[0*pSize] ^ T7;
    T10 = T6 ^ T7;
    T11 = U[6*pSize] ^ U[2*pSize];
    T12 = U[5*pSize] ^ U[2*pSize];
    T13 = T3 ^ T4;
    T14 = T6 ^ T11;
    T15 = T5 ^ T11;
    T16 = T5 ^ T12;
    T17 = T9 ^ T16;
    T18 = U[4*pSize] ^ U[0*pSize];
    T19 = T7 ^ T18;
    T20 = T1 ^ T19;
    T21 = U[1*pSize] ^ U[0*pSize];
    T22 = T7 ^ T21;
    T23 = T2 ^ T22;
    T24 = T2 ^ T10;
    T25 = T20 ^ T17;
...
//    uint pSize = get_global_size(0); // eg. 256
    ...
    __global word_t * Br0 = B + 0 *pSize;
    __global word_t * Br1 = B + 32*pSize;
    __global word_t * Br2 = B + 64*pSize;
    __global word_t * Br3 = B + 96*pSize;
    ...
    uchar offsetr3 = 96;
        ...
        Br0 = B + offsetr0*pSize;
        Br1 = B + offsetr1*pSize;
        Br2 = B + offsetr2*pSize;
        Br3 = B + offsetr3*pSize;


    for (int i = 0; i < 4; i++){
        // B0
        //            2*A0        2*A1              A1           A2           A3
        word_t of =Br0[(R0+7)*pSize]^ Br1[(R1+7)*pSize];
        Bp[A0+0] =                         Br1[(R1+0)*pSize] ^ Br2[(R2+0)*pSize] ^ Br3[(R3+0)*pSize] ^ of;
        Bp[A0+1] = Br0[(R0+0)*pSize] ^ Br1[(R1+0)*pSize] ^ Br1[(R1+1)*pSize] ^ Br2[(R2+1)*pSize] ^ Br3[(R3+1)*pSize] ^ of;
        Bp[A0+2] = Br0[(R0+1)*pSize] ^ Br1[(R1+1)*pSize] ^ Br1[(R1+2)*pSize] ^ Br2[(R2+2)*pSize] ^ Br3[(R3+2)*pSize];
        Bp[A0+3] = Br0[(R0+2)*pSize] ^ Br1[(R1+2)*pSize] ^ Br1[(R1+3)*pSize] ^ Br2[(R2+3)*pSize] ^ Br3[(R3+3)*pSize] ^ of;
        Bp[A0+4] = Br0[(R0+3)*pSize] ^ Br1[(R1+3)*pSize] ^ Br1[(R1+4)*pSize] ^ Br2[(R2+4)*pSize] ^ Br3[(R3+4)*pSize] ^ of;
        Bp[A0+5] = Br0[(R0+4)*pSize] ^ Br1[(R1+4)*pSize] ^ Br1[(R1+5)*pSize] ^ Br2[(R2+5)*pSize] ^ Br3[(R3+5)*pSize];
        Bp[A0+6] = Br0[(R0+5)*pSize] ^ Br1[(R1+5)*pSize] ^ Br1[(R1+6)*pSize] ^ Br2[(R2+6)*pSize] ^ Br3[(R3+6)*pSize];
        Bp[A0+7] = Br0[(R0+6)*pSize] ^ Br1[(R1+6)*pSize] ^ Br1[(R1+7)*pSize] ^ Br2[(R2+7)*pSize] ^ Br3[(R3+7)*pSize];

        //            A0            2*A1        2*A2        A2       A3
        of = Br1[(R1+7)*pSize] ^ Br2[(R2+7)*pSize];
        Bp[A1+0] = Br0[(R0+0)*pSize]                         ^ Br2[(R2+0)*pSize] ^ Br3[(R3+0)*pSize] ^ of;
        Bp[A1+1] = Br0[(R0+1)*pSize] ^ Br1[(R1+0)*pSize] ^ Br2[(R2+0)*pSize] ^ Br2[(R2+1)*pSize] ^ Br3[(R3+1)*pSize] ^ of;
        Bp[A1+2] = Br0[(R0+2)*pSize] ^ Br1[(R1+1)*pSize] ^ Br2[(R2+1)*pSize] ^ Br2[(R2+2)*pSize] ^ Br3[(R3+2)*pSize];
        Bp[A1+3] = Br0[(R0+3)*pSize] ^ Br1[(R1+2)*pSize] ^ Br2[(R2+2)*pSize] ^ Br2[(R2+3)*pSize] ^ Br3[(R3+3)*pSize] ^ of;
        Bp[A1+4] = Br0[(R0+4)*pSize] ^ Br1[(R1+3)*pSize] ^ Br2[(R2+3)*pSize] ^ Br2[(R2+4)*pSize] ^ Br3[(R3+4)*pSize] ^ of;
        Bp[A1+5] = Br0[(R0+5)*pSize] ^ Br1[(R1+4)*pSize] ^ Br2[(R2+4)*pSize] ^ Br2[(R2+5)*pSize] ^ Br3[(R3+5)*pSize];
        Bp[A1+6] = Br0[(R0+6)*pSize] ^ Br1[(R1+5)*pSize] ^ Br2[(R2+5)*pSize] ^ Br2[(R2+6)*pSize] ^ Br3[(R3+6)*pSize];
        Bp[A1+7] = Br0[(R0+7)*pSize] ^ Br1[(R1+6)*pSize] ^ Br2[(R2+6)*pSize] ^ Br2[(R2+7)*pSize] ^ Br3[(R3+7)*pSize];

        //            A0             A1      2*A2        2*A3         A3
        of = Br2[(R2+7)*pSize] ^ Br3[(R3+7)*pSize];
        Bp[A2+0] = Br0[(R0+0)*pSize] ^ Br1[(R1+0)*pSize]                         ^ Br3[(R3+0)*pSize] ^ of;
        Bp[A2+1] = Br0[(R0+1)*pSize] ^ Br1[(R1+1)*pSize] ^ Br2[(R2+0)*pSize] ^ Br3[(R3+0)*pSize] ^ Br3[(R3+1)*pSize] ^ of;
        Bp[A2+2] = Br0[(R0+2)*pSize] ^ Br1[(R1+2)*pSize] ^ Br2[(R2+1)*pSize] ^ Br3[(R3+1)*pSize] ^ Br3[(R3+2)*pSize];
        Bp[A2+3] = Br0[(R0+3)*pSize] ^ Br1[(R1+3)*pSize] ^ Br2[(R2+2)*pSize] ^ Br3[(R3+2)*pSize] ^ Br3[(R3+3)*pSize] ^ of;
        Bp[A2+4] = Br0[(R0+4)*pSize] ^ Br1[(R1+4)*pSize] ^ Br2[(R2+3)*pSize] ^ Br3[(R3+3)*pSize] ^ Br3[(R3+4)*pSize] ^ of;
        Bp[A2+5] = Br0[(R0+5)*pSize] ^ Br1[(R1+5)*pSize] ^ Br2[(R2+4)*pSize] ^ Br3[(R3+4)*pSize] ^ Br3[(R3+5)*pSize];
        Bp[A2+6] = Br0[(R0+6)*pSize] ^ Br1[(R1+6)*pSize] ^ Br2[(R2+5)*pSize] ^ Br3[(R3+5)*pSize] ^ Br3[(R3+6)*pSize];
        Bp[A2+7] = Br0[(R0+7)*pSize] ^ Br1[(R1+7)*pSize] ^ Br2[(R2+6)*pSize] ^ Br3[(R3+6)*pSize] ^ Br3[(R3+7)*pSize];

        //             A0          2*A0           A1       A2      2*A3
        of = Br0[(R0+7)*pSize] ^ Br3[(R3+7)*pSize];
        Bp[A3+0] = Br0[(R0+0)*pSize] ^             Br1[(R1+0)*pSize] ^ Br2[(R2+0)*pSize]             ^ of;
        Bp[A3+1] = Br0[(R0+1)*pSize] ^ Br0[(R0+0)*pSize] ^ Br1[(R1+1)*pSize] ^ Br2[(R2+1)*pSize] ^ Br3[(R3+0)*pSize] ^ of;
        Bp[A3+2] = Br0[(R0+2)*pSize] ^ Br0[(R0+1)*pSize] ^ Br1[(R1+2)*pSize] ^ Br2[(R2+2)*pSize] ^ Br3[(R3+1)*pSize];
        Bp[A3+3] = Br0[(R0+3)*pSize] ^ Br0[(R0+2)*pSize] ^ Br1[(R1+3)*pSize] ^ Br2[(R2+3)*pSize] ^ Br3[(R3+2)*pSize] ^ of;
        Bp[A3+4] = Br0[(R0+4)*pSize] ^ Br0[(R0+3)*pSize] ^ Br1[(R1+4)*pSize] ^ Br2[(R2+4)*pSize] ^ Br3[(R3+3)*pSize] ^ of;
        Bp[A3+5] = Br0[(R0+5)*pSize] ^ Br0[(R0+4)*pSize] ^ Br1[(R1+5)*pSize] ^ Br2[(R2+5)*pSize] ^ Br3[(R3+4)*pSize];
        Bp[A3+6] = Br0[(R0+6)*pSize] ^ Br0[(R0+5)*pSize] ^ Br1[(R1+6)*pSize] ^ Br2[(R2+6)*pSize] ^ Br3[(R3+5)*pSize];
        Bp[A3+7] = Br0[(R0+7)*pSize] ^ Br0[(R0+6)*pSize] ^ Br1[(R1+7)*pSize] ^ Br2[(R2+7)*pSize] ^ Br3[(R3+6)*pSize];
...
void bitsliced_cbc(__global const word_t (* rk)[BLOCK_SIZE], __global word_t *state, ulong len){
//    uint pSize = get_global_size(0); // eg. 256
    uint pSize = get_global_size(0) * get_global_size(1);
    ...
    for (ulong i=0; i+0x10*8<=len*8; i+=0x10*8){
        if (i != 0){
            for (int z=0; z<0x10*8; z++){
                state[(i+z)*pSize] ^= state[(i - 0x10*8 + z)*pSize];
            }
        }

        bs_cipher(&state[i*pSize], rk);
    }
}
...
__kernel void gpu_transpose(__global word_t *input, __global word_t *output, ulong bytelen){
...
//    uint pSize = get_global_size(0); // eg. 256
    ...
    for (ulong i = 0; i<bitlen; i++){
        output[i*pSize + p] = input[i + p*bitlen];
    }
}

__kernel void gpu_transpose_rev(__global word_t *input, __global word_t *output, ulong bytelen){
    ulong bitlen = bytelen*8;
//    uint p = get_global_id(0);
//    uint pSize = get_global_size(0); // eg. 256
    uint p = get_global_id(0) + get_global_id(1) * get_global_size(0);
    ...
    for (ulong i = 0; i<bitlen; i++){
        output[i + p*bitlen] = input[i*pSize + p];
    }
}
...
//    uint pSize = get_global_size(0); // eg. 256
    ...
    __global word_t *dstEnd = &dst[(dstLen * 8)*pSize];
    
    word_t sliced_xor_key[sizeof(uint)*8];
    
    for (int k = 0; k<0x10*8; k++){
        transpose_uint(sliced_xor_key, xorKey);
        for (ulong i = 0; i<inLen * 8; i++) {
            ulong xor_bit_pos = i % (sizeof(uint)*8);
            *dst = input[i*pSize] ^ sliced_xor_key[xor_bit_pos] ^ dst[DERIVATION_BUFFER_SIZE*8*pSize - 0x10*8*pSize]; //xor IV
            dst+=pSize;
        }
        xorKey++;
    }
    ...
    while (dst < dstEnd) {
        ...
        for (ulong i = 0; i<inLen * 8; i++) {
            ulong xor_bit_pos = i % (sizeof(uint)*8);
            *dst = input[i*pSize] ^ sliced_xor_key[xor_bit_pos];
            dst+=pSize;
        }
        xorKey++;
    }
...
void AppleKeyStore_xorCompress_bitsliced(__global word_t* input, uint inputLen, __global word_t* output, uint outputLen){
    ...
    for (ulong i=0; i<inputLen*8; i++) {
        output[(i % (outputLen*8))*pSize] ^= input[i*pSize];
    }
}

__kernel void AppleKeyStore_derivation(__global word_t *data, __global word_t *buffer2, __global word_t *buf1, __global const word_t (* roundkey)[BLOCK_SIZE], uint iter){
//    uint p = get_global_id(0);
//    uint pSize = get_global_size(0); // eg. 256
...
#define dataLength 32
    ...
    while (iter > 0){
        xorkey = AppleKeyStore_xorExpand_bitsliced(buf1+p, DERIVATION_BUFFER_SIZE, buffer2+p, 32, xorkey);
        bitsliced_cbc(roundkey, buf1+p, DERIVATION_BUFFER_SIZE);
        ...
        AppleKeyStore_xorCompress_bitsliced(buf1+p,  r4 * dataLength, data+p, dataLength);
        iter -= r4;
    }
