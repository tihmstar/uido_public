...
bool UIDO::AppleKeyStore_unlockKeybag(uint8_t passcodeKey[32]){
    ...
    KeyBag kbcopy = _kb; //direct access to _kb is not thread safe!
    
    for (int i=0; i < kbcopy.numKeys; i++){
        if (kbcopy.keys[i].wrap & 2){
            if(aes_key_unwrap(&ctx, kbcopy.keys[i].wpky, unwrappedKey, 4))
                return false;
            memcpy(kbcopy.keys[i].wpky, unwrappedKey, 32);
            kbcopy.keys[i].wrap &= ~2;
        }
        if (kbcopy.keys[i].wrap & 1){
            aes128_decrypt(kbcopy.keys[i].wpky, _key835, kbcopy.keys[i].wpky);
            kbcopy.keys[i].wrap &= ~1;
        }
    }
