...
void precompute_tables_inv(void);
 ...
 */
void aes128_precompute_tables(){
    static int didPrecompute = 0;
    if (didPrecompute) return;
...
void aes128_encrypt(const uint8_t plaintext[16], uint8_t roundkeys[11*16], uint8_t ciphertext[16]){
    ...
    for (int i=0; i<16; i++){ //pre key add
        tmp[i] = plaintext[i] ^ roundkeys[0*16+i];
    }
    
    uint32_t w[4] = {0};
    for (int i=1; i<10; i++) { //9 middle rounds

        w[0] = T0[tmp[ 0]] ^ T1[tmp[ 5]] ^ T2[tmp[10]] ^ T3[tmp[15]] ^ *(uint32_t*)&roundkeys[i*16+0];
        w[1] = T0[tmp[ 4]] ^ T1[tmp[ 9]] ^ T2[tmp[14]] ^ T3[tmp[ 3]] ^ *(uint32_t*)&roundkeys[i*16+4];
        w[2] = T0[tmp[ 8]] ^ T1[tmp[13]] ^ T2[tmp[ 2]] ^ T3[tmp[ 7]] ^ *(uint32_t*)&roundkeys[i*16+8];
        w[3] = T0[tmp[12]] ^ T1[tmp[ 1]] ^ T2[tmp[ 6]] ^ T3[tmp[11]] ^ *(uint32_t*)&roundkeys[i*16+12];
        
        *(uint64_t*)&tmp[ 0] = *(uint64_t*)&w[0];
        *(uint64_t*)&tmp[ 8] = *(uint64_t*)&w[2];
    }

    for (int i=0; i<4; i++) { //final round
        ciphertext[4*i + 0] = sbox[tmp[4*i          ]] ^ roundkeys[10*16+4*i + 0];
        ciphertext[4*i + 1] = sbox[tmp[(4*i +  5)%16]] ^ roundkeys[10*16+4*i + 1];
        ciphertext[4*i + 2] = sbox[tmp[(4*i + 10)%16]] ^ roundkeys[10*16+4*i + 2];
        ciphertext[4*i + 3] = sbox[tmp[(4*i + 15)%16]] ^ roundkeys[10*16+4*i + 3];
    }
}

void aes128_decrypt(const uint8_t plaintext[16], uint8_t aeskey[11*16], uint8_t output[16]){
    uint8_t ciphertext[16];
    ciphertext[ 0] = plaintext[ 0] ^ aeskey[16*10 +  0];
    ciphertext[ 1] = plaintext[ 1] ^ aeskey[16*10 +  1];
    ciphertext[ 2] = plaintext[ 2] ^ aeskey[16*10 +  2];
    ciphertext[ 3] = plaintext[ 3] ^ aeskey[16*10 +  3];
    ciphertext[ 4] = plaintext[ 4] ^ aeskey[16*10 +  4];
    ciphertext[ 5] = plaintext[ 5] ^ aeskey[16*10 +  5];
    ciphertext[ 6] = plaintext[ 6] ^ aeskey[16*10 +  6];
    ciphertext[ 7] = plaintext[ 7] ^ aeskey[16*10 +  7];
    ciphertext[ 8] = plaintext[ 8] ^ aeskey[16*10 +  8];
    ciphertext[ 9] = plaintext[ 9] ^ aeskey[16*10 +  9];
    ciphertext[10] = plaintext[10] ^ aeskey[16*10 + 10];
    ciphertext[11] = plaintext[11] ^ aeskey[16*10 + 11];
    ciphertext[12] = plaintext[12] ^ aeskey[16*10 + 12];
    ciphertext[13] = plaintext[13] ^ aeskey[16*10 + 13];
    ciphertext[14] = plaintext[14] ^ aeskey[16*10 + 14];
    ciphertext[15] = plaintext[15] ^ aeskey[16*10 + 15];

    uint32_t w[4] = {0};

    for (int i=1; i<10+1; i++) { //4 middle rounds
        w[0] = T0_inv[ciphertext[ 0]] ^ T1_inv[ciphertext[13]] ^ T2_inv[ciphertext[10]] ^ T3_inv[ciphertext[ 7]]
            ^ IK0[aeskey[(10-i)*16 + 0 ]]
            ^ ((IK0[aeskey[(10-i)*16 + 1 ]]<<8 )   ^ (IK0[aeskey[(10-i)*16 + 1]]>>24))
            ^ ((IK0[aeskey[(10-i)*16 + 2 ]]<<16)   ^ (IK0[aeskey[(10-i)*16 + 2]]>>16))
            ^ ((IK0[aeskey[(10-i)*16 + 3 ]]<<24)   ^ (IK0[aeskey[(10-i)*16 + 3]]>>8));

        w[1] = T0_inv[ciphertext[ 4]] ^ T1_inv[ciphertext[ 1]] ^ T2_inv[ciphertext[14]] ^ T3_inv[ciphertext[11]]
            ^ IK0[aeskey[(10-i)*16 + 4 ]]
            ^ ((IK0[aeskey[(10-i)*16 + 5 ]]<<8 )   ^ (IK0[aeskey[(10-i)*16 + 5]]>>24))
            ^ ((IK0[aeskey[(10-i)*16 + 6 ]]<<16)   ^ (IK0[aeskey[(10-i)*16 + 6]]>>16))
            ^ ((IK0[aeskey[(10-i)*16 + 7 ]]<<24)   ^ (IK0[aeskey[(10-i)*16 + 7]]>>8));

        w[2] = T0_inv[ciphertext[ 8]] ^ T1_inv[ciphertext[ 5]] ^ T2_inv[ciphertext[ 2]] ^ T3_inv[ciphertext[15]]
            ^ IK0[aeskey[(10-i)*16 + 8 ]]
            ^ ((IK0[aeskey[(10-i)*16 + 9 ]]<<8 )   ^ (IK0[aeskey[(10-i)*16 + 9]]>>24))
            ^ ((IK0[aeskey[(10-i)*16 + 10]]<<16)   ^ (IK0[aeskey[(10-i)*16 + 10]]>>16))
            ^ ((IK0[aeskey[(10-i)*16 + 11]]<<24)   ^ (IK0[aeskey[(10-i)*16 + 11]]>>8));

        w[3] = T0_inv[ciphertext[12]] ^ T1_inv[ciphertext[ 9]] ^ T2_inv[ciphertext[ 6]] ^ T3_inv[ciphertext[ 3]]
            ^ IK0[aeskey[(10-i)*16 + 12 ]]
            ^ ((IK0[aeskey[(10-i)*16 + 13]]<<8 )   ^ (IK0[aeskey[(10-i)*16 + 13]]>>24))
            ^ ((IK0[aeskey[(10-i)*16 + 14]]<<16)   ^ (IK0[aeskey[(10-i)*16 + 14]]>>16))
            ^ ((IK0[aeskey[(10-i)*16 + 15]]<<24)   ^ (IK0[aeskey[(10-i)*16 + 15]]>>8));
        
        if (i == 10){ //this is last round
            if (i != 10){
                *(uint32_t*)&ciphertext[ 0] ^= w[0];
                *(uint32_t*)&ciphertext[ 4] ^= w[1];
                *(uint32_t*)&ciphertext[ 8] ^= w[2];
                *(uint32_t*)&ciphertext[12] ^= w[3];
            }
            break;
        }
    ...
    output[ 0] = sbox_inv[ciphertext[ 0]] ^ aeskey[ 0];
    output[ 4] = sbox_inv[ciphertext[ 4]] ^ aeskey[ 4];
    output[ 8] = sbox_inv[ciphertext[ 8]] ^ aeskey[ 8];
    output[12] = sbox_inv[ciphertext[12]] ^ aeskey[12];

    output[ 1] = sbox_inv[ciphertext[13]] ^ aeskey[ 1];
    output[ 5] = sbox_inv[ciphertext[ 1]] ^ aeskey[ 5];
    output[ 9] = sbox_inv[ciphertext[ 5]] ^ aeskey[ 9];
    output[13] = sbox_inv[ciphertext[ 9]] ^ aeskey[13];

    output[ 2] = sbox_inv[ciphertext[10]] ^ aeskey[ 2];
    output[ 6] = sbox_inv[ciphertext[14]] ^ aeskey[ 6];
    output[10] = sbox_inv[ciphertext[ 2]] ^ aeskey[10];
    output[14] = sbox_inv[ciphertext[ 6]] ^ aeskey[14];
    
    output[ 3] = sbox_inv[ciphertext[ 7]] ^ aeskey[ 3];
    output[ 7] = sbox_inv[ciphertext[11]] ^ aeskey[ 7];
    output[11] = sbox_inv[ciphertext[15]] ^ aeskey[11];
    output[15] = sbox_inv[ciphertext[ 3]] ^ aeskey[15];
}

#define Nk 4
#define Nr 10
#define Nb 4
#define getSBoxValue(a) sbox[a]
