...
void bs_addroundkey(word_t * B, __local const word_t * rk){
...
void bs_cipher(word_t *state, __local const word_t (* rk)[BLOCK_SIZE]){
...
void bitsliced_cbc(__local const word_t (* rk)[BLOCK_SIZE], word_t *state, ulong len){
...
uint AppleKeyStore_xorExpand_bitsliced(word_t* dst, uint dstLen, __global const word_t* input, uint inLen, uint xorKey){
    uint pSize = get_global_size(0) * get_global_size(1);
...
void AppleKeyStore_xorCompress_bitsliced(word_t* input, word_t* output, uint len){
    for (ulong i=0; i<len*8; i++) {
        output[i] ^= input[i];
    }
}

__kernel void AppleKeyStore_derivation(__global word_t *data, __global const word_t *gbuffer2, __global const word_t (* roundkey)[BLOCK_SIZE], uint iter){
    uint p = get_global_id(0) + get_global_id(1) * get_global_size(0);
    uint pSize = get_global_size(0) * get_global_size(1);
...
#define dataLength 32
    ...
    word_t pbuf1[dataLength*8];
    word_t ldata[dataLength*8];

    __local word_t lroundkey[11][BLOCK_SIZE];
    
    for (int i=0; i<dataLength*8; i++) ldata[i] = 0;
        
    if (l < 11){
        for (int i=0; i<BLOCK_SIZE; i++){
            lroundkey[l][i] = roundkey[l][i];
        }
    }
    ...
    for (;iter > 0; iter--){
        xorkey = AppleKeyStore_xorExpand_bitsliced(pbuf1, dataLength, gbuffer2+p, dataLength, xorkey);
        for (int i=0; i<0x10*8; i++) pbuf1[i] ^= iv[i];
        bitsliced_cbc(lroundkey, pbuf1, dataLength);
        for (int i=0; i<0x10*8; i++) iv[i] = pbuf1[i+0x10*8];
        AppleKeyStore_xorCompress_bitsliced(pbuf1, ldata, dataLength);
    }

    for (int i=0; i<dataLength*8; i++) data[p + i*pSize] ^= ldata[i];//write data
}
