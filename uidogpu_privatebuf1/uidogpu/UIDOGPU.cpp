...
#define word_t cl_ulong
    ...
    clret = clSetKernelArg(_kernel_AppleKeyStore_derivation, 2, sizeof(cl_mem), &_argRK);assure(!clret);
    {
        cl_ulong iter = get_KeyBag().iter;
        clret = clSetKernelArg(_kernel_AppleKeyStore_derivation, 3, sizeof(cl_uint), &iter);assure(!clret);
    }

    clret = clEnqueueNDRangeKernel(_command_queue, _kernel_AppleKeyStore_derivation, 2, NULL, global_work_size, NULL, 1, &copyBuffer2Event, &appleDerivationEvent);assure(!clret);
