...
//    {
//#warning DEBUG
//        clret = clEnqueueFillBuffer(_command_queue, argKeyBuffer, &zero, 1, 40*1, argKeyBufferSize-40*1, 1, &generateEvent, NULL);assure(!clret);
...
//#warning DEBUG
//    assure(!clFinish(_command_queue));
...
//
//#warning DEBUG
//    _deviceWorkgroupSize = 1024;
//    size_t argKeyBufferSize = 40 * PARALLEL_PINS;
//    uint8_t *debug = (uint8_t*)malloc(argKeyBufferSize);
//    int fd = open("/tmp/kk/dumpall.bin", O_RDONLY);
//    read(fd, debug, argKeyBufferSize);
//    close(fd);
//
//
...
std::string UIDOGPU::bruteforceNumeric(uint64_t startPin, uint64_t endPin){
    ...
    while (curpin < endPin) {
        ...
        keysCnt = generateNumericPasswordBatch(curpin, 4, &keysBuf);
